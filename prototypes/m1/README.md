...
## Contact model, physics version 3
...
- Target maximum speed starts at 650 world units/second; pressure retains the proposed 0.7× speed cap and 1.5× normal contact gain. Response speed scales the cap from 0.5× to 1.5×. Independent smoothing ranges from 0 to 120 ms, initially 30 ms.
...
Acceleration is limited to 12,000 world units/s² (0.7× under pressure) instead of jumping to full velocity. The default target cap is 650 rather than 500, reducing full-width chase lag; arrival still stops at the cursor and keyboard release stops travel. Steering samples direction over a two-unit distance window, rejects subpixel tremor, and eases angular velocity (7 rad/s cap, 70 rad/s² acceleration, 75 ms settling factor). Tiny strokes still finish aligning. Previous/current physics poses are interpolated for drawing, including debris rings, while contacts and scoring remain at 120 Hz. Interpolation adds at most one 8.33 ms physics step of visual delay.

Physics 2 comfort settings migrate; its best time is not compared with physics 3. The new motion checks cover start/reversal acceleration, no-overshoot arrival, tremor resistance and short-stroke turn completion. Subjective smoothness still needs player feedback.
