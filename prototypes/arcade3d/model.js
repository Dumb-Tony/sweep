...
const RoomModel = (() => {
  ...
  function fresh(){return {version:4,removed:Array(COUNT).fill(0),loose:Array(COUNT).fill(0),load:0,floorPhase:0,wallPhase:0,wallStrip:Array(16).fill(0),wallPatch:Array(16).fill(0),wallPaint:Array(16).fill(0),machineDirt:Array.from({length:3},()=>Array(24).fill(1)),stage:0,player:{x:0,z:6,yaw:Math.PI,vx:0,vz:0},cabinets:[{x:-4,z:0,yaw:0,stored:false},{x:0,z:-.5,yaw:0,stored:false},{x:4,z:1.8,yaw:0,stored:false}],held:-1,dust:Array(COUNT).fill(1),grime:Array(COUNT).fill(1),floor:Array(COUNT).fill(0),palette:0,wall:0,color:0,wires:[0,2,3,2,0,1],moon:[0,0,0],pixel:Array(9).fill(0),repaired:[false,false,false],best:[0,0,0]};}
  function valid(s){return !!(s&&s.version===4&&[s.removed,s.loose].every(a=>Array.isArray(a)&&a.length===COUNT&&a.every(v=>Number.isFinite(v)&&v>=0&&v<=1))&&[s.wallStrip,s.wallPatch,s.wallPaint].every(a=>Array.isArray(a)&&a.length===16&&a.every(v=>Number.isFinite(v)&&v>=0&&v<=1))&&Array.isArray(s.machineDirt)&&s.machineDirt.length===3&&s.machineDirt.every(a=>Array.isArray(a)&&a.length===24&&a.every(v=>Number.isFinite(v)&&v>=0&&v<=1))&&Number.isInteger(s.load)&&s.load>=0&&s.load<=12&&Number.isInteger(s.floorPhase)&&s.floorPhase>=0&&s.floorPhase<=2&&Number.isInteger(s.wallPhase)&&s.wallPhase>=0&&s.wallPhase<=2&&(s.trail===undefined||(Array.isArray(s.trail)&&s.trail.length<500&&s.trail.every(p=>Number.isFinite(p.x)&&Number.isFinite(p.z)&&Math.abs(p.x)<9&&p.z> -11&&p.z<9)))&&Number.isInteger(s.stage)&&s.stage>=0&&s.stage<=13&&s.player&&['x','z','yaw'].every(k=>Number.isFinite(s.player[k]))&&Math.abs(s.player.x)<9&&s.player.z>-11&&s.player.z<9&&Number.isInteger(s.held)&&s.held>=-1&&s.held<3&&Array.isArray(s.cabinets)&&s.cabinets.length===3&&s.cabinets.every(c=>Number.isFinite(c.x)&&Number.isFinite(c.z)&&Number.isFinite(c.yaw)&&Math.abs(c.x)<9&&c.z>-11&&c.z<9&&typeof c.stored==='boolean')&&['dust','grime','floor'].every(k=>Array.isArray(s[k])&&s[k].length===COUNT&&s[k].every(v=>Number.isFinite(v)&&v>=0&&v<=1))&&['palette','wall','color'].every(k=>Number.isInteger(s[k])&&s[k]>=0&&s[k]<4)&&Array.isArray(s.wires)&&s.wires.length===6&&s.wires.every(v=>Number.isInteger(v)&&v>=0&&v<4)&&Array.isArray(s.moon)&&s.moon.length===3&&s.moon.every(v=>Number.isInteger(v)&&v>=0&&v<4)&&Array.isArray(s.pixel)&&s.pixel.length===9&&s.pixel.every(v=>v===0||v===1)&&Array.isArray(s.repaired)&&s.repaired.length===3&&s.repaired.every(v=>typeof v==='boolean')&&Array.isArray(s.best)&&s.best.length===3&&s.best.every(v=>Number.isFinite(v)&&v>=0));}
  ...
  function migrate(old){if(!old)return null;if(old.version===4){if(!valid(old))return null;while(workKey(old)&&!remaining(old).length)work(old);return old;}if(old.version===3){const n=Object.assign(fresh(),old,{version:4,moon:[0,0,0],pixel:Array(9).fill(0),repaired:[!!old.repaired,false,false],best:[Number(old.best)||0,0,0]});return valid(n)?n:null;}if(old.version!==2)return null;const n=Object.assign(fresh(),old,{version:4,repaired:Array.isArray(old.repaired)?old.repaired:[!!old.repaired,false,false],best:Array.isArray(old.best)?old.best:[Number(old.best)||0,0,0],moon:Array.isArray(old.moon)?old.moon:[0,0,0],pixel:Array.isArray(old.pixel)?old.pixel:Array(9).fill(0)});if(n.stage>=3){n.floorPhase=2;n.removed.fill(1);}if(n.stage>=5){n.wallPhase=2;n.wallStrip.fill(1);n.wallPatch.fill(1);n.wallPaint.fill(1);n.machineDirt.forEach(a=>a.fill(0));}if(n.stage>=6)n.repaired[0]=true;return valid(n)?n:null;}  function workKey(s){return s.stage===1?'dust':s.stage===2?'grime':s.stage===3?(s.floorPhase===0?'removed':s.floorPhase===2?'floor':null):s.stage===4?['wallStrip','wallPatch','wallPaint'][s.wallPhase]:null;}
  function progress(s){if(s.stage===0)return s.cabinets.filter(c=>c.stored).length/3;if(s.stage===3&&s.floorPhase===1)return 1-(s.loose.reduce((a,v)=>a+v,0)+s.load)/COUNT;if(s.stage===5)return 1-mean(s.machineDirt.flat());const key=workKey(s);if(key)return s.stage<3?1-mean(s[key]):mean(s[key]);return s.stage===13?1:0;}
  ...
  function interact(s){
    ...
    if(s.held>=0){const i=s.held,c=s.cabinets[i];if(s.stage===0&&s.player.z< -6.3){c.x=bays[i].x;c.z=bays[i].z;c.yaw=0;c.stored=true;s.held=-1;if(Math.abs(s.player.x-c.x)<1&&Math.abs(s.player.z-c.z)<1)s.player.z=c.z+1.5;if(s.cabinets.every(c=>c.stored))s.stage=1;return 'stored';}
      if(((s.stage===6&&i===0)||(s.stage===8&&i===1)||(s.stage===11&&i===2))&&s.player.z> -3.5){c.x=s.player.x;c.z=s.player.z-1.5;c.yaw=0;c.stored=false;s.held=-1;s.stage++;return 'restored';}
      s.held=-1;return 'dropped';}
    const i=nearest(s);if(i<0)return 'nearer';if((s.stage===0&&!s.cabinets[i].stored)||(s.stage===6&&i===0)||(s.stage===8&&i===1)||(s.stage===11&&i===2)){s.held=i;s.trail=[];trails.delete(s);return 'loaded';}if(s.stage===5&&s.machineDirt[i].some(v=>v>0))return 'clean';if((s.stage===5&&i===0&&s.machineDirt.every(a=>a.every(v=>v===0)))||(s.stage===7&&i===1)||(s.stage===10&&i===2))return 'repair';if((s.stage===7&&i===0)||(s.stage===9&&i===1)||(s.stage===12&&i===2)||(s.stage===13&&s.repaired[i]))return 'play';return 'unavailable';
  }
  ...
  function repair(s,i=0){const solved=i===0&&s.stage===5&&s.machineDirt.every(a=>a.every(v=>v===0))&&s.wires.every((v,j)=>v===solution[j])||i===1&&s.stage===7&&s.moon.join()==='2,1,3'||i===2&&s.stage===10&&s.pixel.every(Boolean);if(solved){s.repaired[i]=true;s.stage++;return true;}return false;} function commission(s,i){if((s.stage===9&&i===1)||(s.stage===12&&i===2)){s.stage++;return true;}return false;}
  return {fresh,valid,migrate,workKey,wallTile,wipe,cameraPosition,cameraBlocked,blocked,progress,percent,remaining,work,step,nearest,interact,path,repair,commission,bays,tile,COUNT,solution};
})();
